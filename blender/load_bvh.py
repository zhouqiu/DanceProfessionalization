...
class BVH_file:
    def __init__(self, file_path, pos=0.0, multi=1.0):#pos -1 0 1
        ...
        self.normalize(pos = pos, multi=multi)
    ...
    def normalize(self, pos=0.0, multi=1.0):
        ...
        self.anim.positions[:, 0, 1] -= mean_position[1] + pos * 10 * multi
        self.anim.positions[:, 0, 1] -= mean_position[1] + 1
...
def set_offset(num):
    offset = np.linspace(0,num-1,num)
    offset -=(num-1)/2
    offset = offset[::-1]
    return offset

def load_bvh_allpath(path_list,multi=1.0):#list
    num = len(path_list)
    offsets = set_offset(num)
    all_objs = []
    for i in range(num):
        print('Loading BVH file......')
        file = BVH_file(path_list[i], offsets[i], multi=multi)
        print(" filename:{}".format(path_list[i]))
        ...
        print('Loading keyframes{} done.'.format(i+1))
        bpy.ops.object.select_all(action='DESELECT')
        for obj in all_obj:
            obj.select_set(True)
        bpy.ops.object.move_to_collection(collection_index=0, is_new=True, new_collection_name="Character"+str(i+1))
        bpy.ops.object.select_all(action='DESELECT')
        print('Load bvh{} all done!'.format(i+1))
