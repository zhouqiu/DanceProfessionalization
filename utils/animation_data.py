...
class AnimationData:
    ...
    @classmethod
    def from_network_output(cls, input, frametime=1/30):
        input = input.transpose(1, 0)
        input = np.concatenate((input, np.zeros((len(input), 4))), axis=-1)
        return cls(input, frametime=frametime)
    ...
    def from_rotations_and_root_positions(cls, rotations, root_positions, skel=None, frametime=1/30):
        ...
        root_positions: [T, 3]
        """
        # print("from_rotations_and_root_positions")
        # print(rotations.shape)
        # print(root_positions.shape)
        if skel is None:
            skel = Skel()
